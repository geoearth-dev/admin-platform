...
import type { GenerateType } from './script'

import type { ComponentConfig, FormConfig } from './types'

export interface HtmlField extends ComponentConfig {
  layout: 'colFormItem' | 'rowFormItem'
  children?: HtmlField[]
}

export interface HtmlFormConfig extends FormConfig {
  fields: HtmlField[]
}

type FieldRenderer = (field: HtmlField) => string

const ruleTriggers: Readonly<Partial<Record<string, string>>> = trigger

let confGlobal: HtmlFormConfig | null = null
...
function buildFromBtns(conf: HtmlFormConfig, type: GenerateType): string {
  let str = ''
...
// span不为24的用el-col包裹
function colWrapper(element: HtmlField, str: string): string {
...
const layouts: Record<HtmlField['layout'], FieldRenderer> = {
  colFormItem(element) {
    let labelWidth = ''
    ...
    const required =
      !(element.tag && ruleTriggers[element.tag]) && element.required
        ? 'required'
        : ''
    const renderTag = element.tag ? tags[element.tag] : undefined
    const tagDom = renderTag ? renderTag(element) : null
...
const tags: Partial<Record<string, FieldRenderer>> = {
  'el-button': (el) => {
...
function attrBuilder(el: HtmlField) {
  return {
...
// el-buttin 子级
function buildElButtonChild(conf: HtmlField): string {
...
// el-input innerHTML
function buildElInputChild(conf: HtmlField): string {
  const children: string[] = []
  if (conf.prepend) {
...
function buildElSelectChild(conf: HtmlField): string {
  const children: string[] = []
...
function buildElRadioGroupChild(conf: HtmlField): string {
  const children: string[] = []
  if (conf.options && conf.options.length) {
...
function buildElCheckboxGroupChild(conf: HtmlField): string {
  const children: string[] = []
  if (conf.options && conf.options.length) {
    const tag =
...
function buildElUploadChild(conf: HtmlField): string {
  const list: string[] = []
...
export function makeUpHtml(conf: HtmlFormConfig, type: GenerateType): string {
