import { LocalStorageDriver } from "./driver/LocalStorageDriver";
import { MemoryStorageDriver } from "./driver/MemoryStorageDriver";
import type { IStorageDriver, StorageItem, StorageManagerOptions } from "./type";
...
class StorageManager {
  ...
  constructor({ driver, prefix = '' }: StorageManagerOptions = {}) {
    ...
    if (!this.prefix && this.driver instanceof LocalStorageDriver) {
      console.warn('[StorageManager] empty prefix combined with LocalStorageDriver — clear()/keys() will affect every localStorage entry.',);
    }
  }
