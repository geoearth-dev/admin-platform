...
import { ChevronsDown, CircleAlert } from '@/assets/icons'
import {
  Button,
  FormControl,
  FormDescription,
...
import { isFunction, isObject, isString } from '@/utils/inference'
...
interface RuntimeFieldSlotProps {
  field: FormRuntimeField<unknown>
}
...
async function validateFieldValue({ value }: { value: unknown }) {
  ...
  if (isString(activeRules)) {
    ...
    // 返回 false 的自定义规则也表示失败，转换为运行时可展示的错误。
    return result === true
      ? undefined
      : result === false
        ? `${ruleContext.label ?? fieldName}校验失败`
        : result
  }
...
const computedProps = computed(() => {
  const commonProps = isFunction(commonComponentProps)
    ? commonComponentProps({ fieldName })
    : commonComponentProps
...
function fieldBindEvent(componentField: FormComponentField) {
  const modelValue = componentField.modelValue
  // 类型明确为 (value: unknown) => void
  ...
  if (bindEventField) {
    // 先识别事件，再处理普通对象
    if (isEventObjectLike(modelValue)) {
      value = modelValue.target[bindEventField]
    } else if (isObject(modelValue)) {
      value = modelValue[bindEventField] ?? modelValue
    }
  }
  ...
  if (bindEventField) {
    ...
    // 组件可能直接传值，也可能传事件
    function handleChangeEvent(event: unknown) {
  ...
  return {
    modelValue: modelValue === undefined ? emptyStateValue : modelValue,
