...
import { useForwardPriorityValues } from '@/plugins/composables'
import { isEqual } from 'es-toolkit/compat'

import { useDebounceFn } from '@vueuse/core'

import FormActions from './components/form-actions.vue'
import { getValueByFieldName } from './field-name'
import {
...
const handleValuesChangeDebounced = useDebounceFn(
  async () => {
    if (formApi.isMounted && state.value.submitOnChange) {
...
watch(values, (currentValues, previousValues) => {
  ...
  if (handleValuesChange && fields.length > 0) {
    const changedFields = fields.filter((field) => {
      return !isEqual(
        getValueByFieldName(currentValues, field),
        getValueByFieldName(previousValues ?? {}, field),
      )
    })
