...
const shouldShowText = computed(() => {
  return props.showText && !props.collapsed && !shouldUseFullLogo.value && !!props.text;
});
...
const fullLogoStyle = computed(() => ({
  height:
    typeof props.fullLogoHeight === 'number' ? `${props.fullLogoHeight}px` : props.fullLogoHeight,
  objectFit: props.fit,
}));
</script>
...
<template>
  <div :class="theme" class="flex h-full items-center text-lg">
    <a
      :class="[
        $attrs.class,
        shouldShowText ? 'gap-2 px-3 justify-start' : 'w-full p-0 justify-center',
      ]"
      :href="href"
