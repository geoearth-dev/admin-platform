...
const emits = defineEmits<{ 'update:value': [unknown, string] }>();
...
async function onParamValueChange(_: unknown, key: string) {
  await nextTick();
...
<template>
  <CollapsibleRoot
    v-model:open="open"
    class="border rounded-xl flex flex-col w-full overflow-hidden"
    :class="[b()]"
