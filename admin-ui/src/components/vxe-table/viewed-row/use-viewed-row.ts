...
import { isBoolean, isFunction } from 'es-toolkit/compat'
...
type RowParams<T> = Parameters<
  Exclude<VxeTablePropTypes.RowClassName<T>, string>
>[0]
type RowStyleResult<T> = ReturnType<
  Extract<VxeTablePropTypes.RowStyle<T>, (...args: never[]) => unknown>
>
...
function createStorageAdapter(
  ...
  switch (persist.type) {
    ...
    case 'sessionStorage': {
      return createWebStorageAdapter('sessionStorage', persist.key, persist.ttl)
    }
    default: {
      return null
    }
...
export function useViewedRow<T extends object = Record<string, unknown>>(
  ...
  function persistImmediate() {
    if (!adapter) return
    const keys = [...viewedSet.value]
    enqueueWrite(() => adapter.setKeys(keys))
  }
  function persist() {
    if (!adapter) return
    clearTimeout(timer)
  ...
  async function restoreFromStorage(): Promise<void> {
    ...
    try {
      ...
      if (!restoreCancelled && stored.length > 0) {
  ...
  restoreFromStorage().then(() => {
    if (!restoreCancelled && options.viewedKeys) {
  ...
  let stopViewedKeys: (() => void) | undefined
  if (isRef(options.viewedKeys)) {
    stopViewedKeys = watch(options.viewedKeys, (newKeys) => {
      updateViewedSet((set) => {
        let changed = false
  ...
  function markAsViewed(record: T) {
    const key = (record as Record<string, unknown>)[options.keyField]
  ...
  function clearViewed() {
    ...
    if (adapter) {
      enqueueWrite(() => adapter.removeKeys())
    }
  }
  ...
  // ========== rowClassName 函数 ==========
  function getRowClassName(params: RowParams<T>): string {
  ...
  // ========== rowStyle 函数 ==========
  function getRowStyle(params: RowParams<T>): RowStyleResult<T> {
...
export type ViewedRowHelper<T extends object = Record<string, unknown>> =
  ReturnType<typeof useViewedRow<T>>
...
function mergeClassNames(...classNames: unknown[]): string {
  return classNames
 ...
 */
function wrapColumnsForViewedRow<T extends object>(
  ...
  return columns.map((column) => {
    ...
    if (nextColumn.cellRender?.name === 'CellOperation') {
      ...
      attrs.onClick = (params: { code: string; row: T }) => {
 ...
 */
export function applyViewedRowOptions<T extends object>(
  mergedOptions: VxeTableGridProps<T>,
  viewedRowConfig: boolean | ViewedRowOptions<T>,
  helper: ViewedRowHelper<T>,
) {
  ...
  mergedOptions.rowClassName = (params) => {
  ...
  mergedOptions.rowStyle = (params) => {
    ...
    let viewedStyle: RowStyleResult<T>
