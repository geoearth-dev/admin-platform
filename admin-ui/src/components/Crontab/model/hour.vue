...
const rangeExpression = computed(() => {
  const start = props.check(rangeStart.value ?? 0, 0, 22)
  const end = props.check(rangeEnd.value ?? start + 1, start + 1, 23)
  return `${start}-${end}`
})

const intervalExpression = computed(() => {
  const start = props.check(intervalStart.value ?? 0, 0, 22)
  const step = props.check(intervalStep.value ?? 1, 1, 23 - start)
  return `${start}/${step}`
})

const selectedExpression = computed(() => selectedValues.value.join(','))

watch(() => props.cron.hour, readExpression)
watch(
...
function updateExpression(): void {
  switch (mode.value) {
    case 'every':
      ...
      break
    case 'range':
      emit('update', 'hour', rangeExpression.value, 'hour')
      break
    case 'interval':
      emit('update', 'hour', intervalExpression.value, 'hour')
      break
    case 'specific': {
      // 清空多选时，保留上次选中的第一个值。
      ...
      emit('update', 'hour', selectedExpression.value, 'hour')
      break
    }
  }
}
