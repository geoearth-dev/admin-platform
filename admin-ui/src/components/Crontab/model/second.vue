...
watch(() => props.cron.second, readExpression)
watch(
...
function updateExpression(): void {
  switch (mode.value) {
    case 'every':
      ...
      break
    case 'range':
      emit('update', 'second', rangeExpression.value, 'second')
      break
    case 'interval':
      emit('update', 'second', intervalExpression.value, 'second')
      break
    case 'specific': {
      // 清空多选时，保留上次选中的第一个值。
      ...
      emit('update', 'second', selectedExpression.value, 'second')
      break
    }
  }
}
