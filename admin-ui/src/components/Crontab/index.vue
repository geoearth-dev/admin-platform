...
import CrontabSecond from './model/second.vue';
import CrontabMin from './model/min.vue';
import CrontabHour from './model/hour.vue';
import CrontabDay from './model/day.vue';
import CrontabMonth from './model/month.vue';
import CrontabWeek from './model/week.vue';
import CrontabYear from './model/year.vue';
import CrontabResult from './result.vue';
import { computed, onMounted, ref, watch } from 'vue';
import type { CrontabField, CrontabValue } from './type.ts';
...
function resolveExp(): void {
  const expressionValue = props.expression.trim();
  if (!expressionValue) {
    clearCron();
    return;
  }
  const arr = expressionValue.split(/\s+/);
  if (arr.length >= 6) {
    //6 位以上是合法表达式
