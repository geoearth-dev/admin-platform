...
import type { DateArrays, DayRule } from './type';
...
const dayRule = ref<DayRule>('');
const dayRuleSup = ref<'' | 'null' | number | number[]>('');
const dateArr = ref<DateArrays>([[], [], [], [], [], []]);
...
function expressionChange(): void {
  ...
  const ruleArr = props.ex.trim().split(/\s+/);
  if (ruleArr.length < 6 || ruleArr.length > 7) {
    resultList.value = ['请输入有效的 Cron 表达式'];
    isShow.value = true;
    return;
  }
  // 用于记录进入循环的次数
  ...
  if (
    dateArr.value.some(
      (values) => values.length === 0 || values.some((value) => !Number.isFinite(value)),
    )
  ) {
  ...
  const ruleSup = dayRuleSup.value;
  // 将获取到的数组赋值-方便使用
    ...
    goMonth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
      ...
      goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
        ...
        if (dayRule.value === 'lastDay') {
          ...
          if (!Array.isArray(ruleSup) || !ruleSup.includes(thisWeek)) {
            // 如果到达最大值时
            if (Di === DDate.length - 1) {
              resetDay();
              if (Mi === MDate.length - 1) {
          ...
          if (!Array.isArray(ruleSup)) continue;
          const [occurrence, weekday] = ruleSup;
          if (occurrence === undefined || weekday === undefined) continue;
          // 如果指定了是第几周的星期几
          // 获取每月1号是属于星期几
          const thisWeek = formatDate(new Date(YY + '-' + MM + '-' + DD + ' 00:00:00'), 'week');
          if (weekday >= thisWeek) {
            DD = (occurrence - 1) * 7 + weekday - thisWeek + 1;
          } else {
            DD = occurrence * 7 + weekday - thisWeek + 1;
          }
        } else if (dayRule.value === 'lastWeek') {
          if (typeof ruleSup !== 'number') continue;
          // 如果指定了每月最后一个星期几
          // 校验并调整如果是2月30号这种日期传进来时需调整至正常月底
          if (checkDate(YY + '-' + MM + '-' + thisDD + ' 00:00:00') !== true) {
            while (DD > 0 && checkDate(YY + '-' + MM + '-' + thisDD + ' 00:00:00') !== true) {
          ...
          if (ruleSup < thisWeek) {
            DD -= thisWeek - ruleSup;
          } else if (ruleSup > thisWeek) {
            DD -= 7 - (ruleSup - thisWeek);
          }
        }
        // 判断时间值是否小于10置换成“05”这种格式
        const dayText = pad2(DD);
        // 循环“时”数组
        goHour: for (let hi = hIdx; hi < hDate.length; hi++) {
          const hh = pad2(hDate[hi]!);
          // 如果到达最大值时
          ...
          goMin: for (let mi = mIdx; mi < mDate.length; mi++) {
            ...
            for (let si = sIdx; si <= sDate.length - 1; si++) {
              const ss = pad2(sDate[si]!);
              // 添加当前时间（时间合法性在日期循环时已经判断）
              if (MM !== '00' && dayText !== '00') {
                resultArr.push(YY + '-' + MM + '-' + dayText + ' ' + hh + ':' + mm + ':' + ss);
                nums++;
              }
              // 如果条数满了就退出循环
...
function getWeekArr(rule: string): void {
  ...
  if (dayRule.value === '' && dayRuleSup.value === '') {
    if (rule.indexOf('-') >= 0) {
      ...
      const matchRule = rule.match(/^([1-7])#([1-5])$/);
      if (!matchRule) {
        dateArr.value[3] = [];
        return;
      }
      dayRuleSup.value = [Number(matchRule[2]), Number(matchRule[1])];
      dateArr.value[3] = [1];
      ...
      dayRule.value = 'lastWeek';
      const matchRule = rule.match(/^([1-7])L$/);
      if (!matchRule) {
        dateArr.value[3] = [];
        return;
      }
      dayRuleSup.value = Number(matchRule[1]);
      dateArr.value[3] = [31];
...
function getDayArr(rule: string): void {
  ...
  if (rule.indexOf('-') >= 0) {
    ...
    dayRule.value = 'workDay';
    const matchRule = rule.match(/^(\d{1,2})W$/);
    if (!matchRule) {
      dateArr.value[3] = [];
      return;
    }
...
function getAverageArr(rule: string, limit: number): number[] {
  ...
  if (!Number.isInteger(min) || !Number.isInteger(step) || step <= 0) return [];
  while (min <= limit) {
    arr.push(min);
    min += step;
  }
...
function formatDate(value: number | Date, type: 'week'): number;
function formatDate(value: number | Date, type?: 'week'): string | number {
  // 计算日期相关值
