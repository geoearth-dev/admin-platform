      ...
      <el-col :span="12">
        <el-form-item prop="packageName">
          <template #label>
            生成包路径
            <el-tooltip
              content="生成在哪个java包下，例如 dev.geo.admin.system"
              placement="top"
            >
...
const props = defineProps<{ info: GenTable; tables: GenTable[] }>()
const info = computed(() => props.info)
const menuOptions = ref<TreeSelect[]>([])
const genInfoForm = ref<FormInstance>()
const subColumns = computed(
  () =>
    props.tables.find((table) => table.tableName === props.info.subTableName)
      ?.columns ?? [],
)
const rules: FormRules<GenTable> = {
  tplCategory: [
    { required: true, message: '请选择生成模板', trigger: 'change' },
  ],
  packageName: [
...
async function validate(): Promise<boolean> {
  return (await genInfoForm.value?.validate().catch(() => false)) ?? false
}
onMounted(async () => {
  menuOptions.value = [
    { id: 0, label: '顶级菜单', disabled: false, children: await treeselect() },
  ]
})
