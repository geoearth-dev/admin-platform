...
import type { MenuSaveParams, MenuType, SysMenu } from '@/types/base/api/system/menu';
...
export function toMenuSaveParams(values: MenuFormValues, id?: number): MenuSaveParams {
  ...
  return {
    ...
    badgeVariants: route && values.badgeType ? values.badgeVariants || 'default' : null,
    keepAlive: page && !!values.keepAlive,
...
export function parseMenuQuery(text?: string | null): LocationQueryRaw | null {
  ...
  if (
    ...
    !Object.values(query).every(
      (value) => isScalar(value) || (Array.isArray(value) && value.every(isScalar)),
    )
  ) {
    throw new Error('查询参数必须是对象，值只能为字符串、数字、null 或这些值组成的数组');
  }
