...
import { ElButton, ElCard, ElMessage, ElSpace, ElTag } from 'element-plus'
...
interface ValueFormatFormValues extends Record<string, unknown> {
  deadline?: Date | null
  keyword?: string
  reportRange?: [Date, Date] | null
}
...
function decodeValueFormatValues(
  ...
  let reportRange: [Date, Date] | undefined
  if (values.startTime !== undefined && values.endTime !== undefined) {
    reportRange = [new Date(values.startTime), new Date(values.endTime)]
  }
  return {
    deadline:
      values.deadline === undefined ? undefined : new Date(values.deadline),
    keyword: values.keyword,
    reportRange,
  }
}
  ...
  return JSON.stringify(
    ...
    (_key, currentValue) => {
      return currentValue instanceof Date
        ? dayjs(currentValue).format('YYYY-MM-DD HH:mm:ss')
        : currentValue
    },
    2,
  )
...
async function handleInspectValues() {
  ...
  ElMessage.success('已刷新 getValues 输出')
}

function handleSetExampleValue() {
  formApi.setValues({
    deadline: dayjs('2026-04-12 18:30:00').toDate(),
    keyword: 'invoice',
    reportRange: [
      dayjs('2026-04-01 00:00:00').toDate(),
      dayjs('2026-04-12 23:59:59').toDate(),
    ],
  })
}
...
function handleSubmit(values: ValueFormatSubmitValues) {
  ...
  ElMessage.success({
    message: `getValues output: ${JSON.stringify(values)}`,
  })
}
...
<template>
  ...
  >
    <template #description>
      <div class="text-muted-foreground space-y-2">
        ...
        <div class="flex flex-wrap gap-2">
          <ElTag type="primary">encode：生成完整提交值</ElTag>
          <ElTag type="success">decode：恢复完整表单值</ElTag>
          <ElTag type="warning">多字段转换原子执行</ElTag>
        </div>
      </div>
    </template>
    <template #extra>
    ...
    <ElCard header="Codec 示例">
      <template #header
        ><div class="flex items-center justify-between gap-3">
          <span>Codec 示例</span>
          <div>
            <ElSpace wrap>
              <ElButton @click="handleSetExampleValue">填充示例数据</ElButton>
              <ElButton
                type="primary"
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      <ElCard header="getRawValues() 输出（组件值）">
        <pre class="bg-muted overflow-auto rounded-md p-4 text-sm">{{
          liveValuesPreview
        }}</pre>
      </ElCard>
      <ElCard header="getValues / submit 输出（codec.encode 后）">
        <pre class="bg-muted overflow-auto rounded-md p-4 text-sm">{{
