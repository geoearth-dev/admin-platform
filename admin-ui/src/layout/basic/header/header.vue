...
import { LockKeyhole, LogOut, RotateCw } from '@/assets/icons';
import { $t } from '@/plugins/locale';
import { usePreferences, preferences } from '@/plugins/preference';
import { useAccessStore } from '@/store';
import { useVbenModal } from '@/plugins/vben-ui/popup-ui';

import { VbenFullScreen, VbenIconButton, VbenTooltip } from '@/plugins/vben-ui/shadcn-ui';
...
const showLockInHeader = computed(
  () => preferences.widget.lockScreen && preferences.widget.lockScreenButtonPosition === 'header',
);

const showLogoutInHeader = computed(() => preferences.widget.logoutButtonPosition === 'header');
...
const rightSlots = computed(() => {
  ...
  const widgetChecks: Record<string, { slotName: string; visible: boolean }> = {
    ...
    themeToggle: {
      visible:
        preferences.widget.themeToggle && preferences.widget.themeToggleButtonPosition === 'header',
    ...
    timezone: {
      visible:
        preferences.widget.timezone && preferences.widget.timezoneButtonPosition === 'header',
      slotName: 'timezone',
    },
    fullscreen: {
      visible:
        preferences.widget.fullscreen && preferences.widget.fullscreenButtonPosition === 'header',
      slotName: 'fullscreen',
    },
    refresh: {
      visible: preferences.widget.refresh && preferences.widget.refreshButtonPosition === 'header',
      slotName: 'refresh',
    },
    notification: {
      visible:
...
function nextIndex(list: Array<SlotItem>) {
  const index = list.length > 0 ? Math.max(...list.map((item) => item.index)) : 0;
  return index + 1;
}
...
<template>
  <LockModal v-if="showLockInHeader" :avatar="avatar" :text="text" @submit="handleSubmitLock" />
    ...
    <template v-for="slot in rightSlots" :key="slot.name">
      <slot :name="slot.name">
        ...
        <template v-else-if="slot.name === 'fullscreen'">
          <VbenFullScreen class="mr-1" :tooltip="$t('preferences.widget.fullscreen')" />
        </template>
        ...
        <template v-else-if="slot.name === 'logout-btn'">
          <VbenIconButton class="mr-1" :tooltip="$t('common.logout')" @click="handleLogout">
            <LogOut class="size-4" />
          </VbenIconButton>
        </template>
