  ...
  function hasAccess(route: RouteRecordRaw): boolean {
    ...
    if (route.meta?.permissions) {
      return route.meta.permissions.some(
        (permission) =>
          permissions.has(ALL_PERMISSION) || permissions.has(permission),
      )
    }

    if (route.meta?.authority) {
      return route.meta.authority.some(
        (role) => roles.has('admin') || roles.has(role),
      )
    }
...
async function generateAccess(): Promise<GenerateAccessResult> {
  ...
  const accessibleRoutes = completeMenuRedirects([
    ...backendRoutes,
    ...localRoutes,
  ])
